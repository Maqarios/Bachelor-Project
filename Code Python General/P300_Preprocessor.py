...
class P300_Preprocessor(object):
    ...
    def calculate_average(self):
        ...
        preprocessed_signal = numpy.zeros((self.preprocessed_signal.shape[0], self.intensifications, self.window, self.preprocessed_signal.shape[2]))
        ...
        for epoch in range(self.preprocessed_signal.shape[0]):
            ...
            for n in range(1, self.preprocessed_signal.shape[1]):
                if self.stimulus_code[epoch, n] == 0 and self.stimulus_code[epoch, n - 1] != 0:
                    ...
                    try:
                        preprocessed_signal[epoch, int(self.stimulus_code[epoch, n - 1]) - 1] += self.preprocessed_signal[epoch, n + self.start_window - self.digitization_difference : n + self.end_window - self.digitization_difference]
                    except:
                        intensification_counter[int(self.stimulus_code[epoch, n - 1]) - 1] -= 1
    ...
    def calulate_common_average_reference(self):
        ...
        for epoch in range(self.preprocessed_signal.shape[0]):
            ...
            for channel in range(self.preprocessed_signal.shape[3]):
                self.preprocessed_signal[epoch, :, :, channel] = self.preprocessed_signal[epoch, :, :, channel] - mean_sample
    ...
    def calculate_z_score(self):
        ...
        for epoch in range(self.preprocessed_signal.shape[0]):
            ...
            for sample in range(self.preprocessed_signal.shape[2]):
                self.preprocessed_signal[epoch, :, sample, :] = (self.preprocessed_signal[epoch, :, sample, :] - mean_channel) / std_channel
    ...
    def calculate_notch(self):
        ...
        for epoch in range(self.preprocessed_signal.shape[0]):
            for intensification in range(self.preprocessed_signal.shape[1]):
                for channel in range(self.preprocessed_signal.shape[3]):
                    self.preprocessed_signal[epoch, intensification, :, channel] = signal.lfilter(b, a, self.preprocessed_signal[epoch, intensification, :, channel])
    
    # Calculation of Decimation
    def calculate_decimation(self, decimation):
        
        preprocessed_signal = numpy.zeros((self.preprocessed_signal.shape[0], self.intensifications, math.floor(self.window / decimation), self.preprocessed_signal.shape[3]))
        ...
        for epoch in range(self.preprocessed_signal.shape[0]):
            ...
            for intensification in range(self.preprocessed_signal.shape[1]):
                for i in range(math.floor(self.window / decimation)):
                    preprocessed_signal[epoch, intensification, i, :] = self.preprocessed_signal[epoch, intensification, (i * decimation), :]
    ...
    def calculate_concatenation(self, extracted_channels):
        ...
        preprocessed_signal = numpy.zeros((self.preprocessed_signal.shape[0], self.preprocessed_signal.shape[1], self.preprocessed_signal.shape[2] * extracted_channels.shape[0]))
        ...
        for epoch in range(self.preprocessed_signal.shape[0]):
            ...
            for intensification in range(self.preprocessed_signal.shape[1]):
                    ...
                    for channel_index in range(extracted_channels.shape[0]):
                        preprocessed_signal[epoch, intensification, (self.preprocessed_signal.shape[2] * channel_index):(self.preprocessed_signal.shape[2] * channel_index) + self.preprocessed_signal.shape[2]] = \
